import{a as F}from"./chunk-4Q7TQKNT.js";import{f as P,g as N,h as H,m as I}from"./chunk-O4XQ3THW.js";import{Da as q,Ka as r,La as t,Ma as f,Na as M,Oa as v,Pa as g,Qa as O,Ra as A,Sa as D,Ta as c,Ua as w,V as y,W as h,Wa as C,a as x,b as z,da as S,la as _,na as d,ra as k,va as E,ya as b}from"./chunk-XKHWOHTO.js";var R=["starsCanvas"];function L(i,a){if(i&1){let e=M();r(0,"div",12),v("click",function(){let o=y(e).$implicit,n=g(2);return h(n.abrirModal(o))}),c(1),t()}if(i&2){let e=a.$implicit;d(),C(" ",e.diaSemana," ",e.dia," ")}}function T(i,a){if(i&1&&(r(0,"div",8)(1,"div",9),c(2),t(),r(3,"div",10),b(4,L,2,2,"div",11),t()()),i&2){let e=a.$implicit;d(2),C("",e.mes," ",e.anio,""),d(2),q("ngForOf",e.items)}}function B(i,a){if(i&1){let e=M();r(0,"div",13),v("click",function(){y(e);let o=g();return h(o.cerrarModal())}),r(1,"div",14),v("click",function(o){return y(e),h(o.stopPropagation())}),r(2,"div",15),c(3),t(),r(4,"div",16),c(5),t(),f(6,"div",17),r(7,"button",18),v("click",function(){y(e);let o=g();return h(o.cerrarModal())}),c(8,"Cerrar"),t()()()}if(i&2){let e=g();d(3),w(e.entradaAbierta.fecha),d(2),w(e.entradaAbierta.titulo),d(),q("innerHTML",e.entradaAbierta.texto,_)}}var V=class i{constructor(a){this.http=a}canvasRef;ctx;stars=[];animationId;width=window.innerWidth;height=window.innerHeight;ngAfterViewInit(){this.initCanvas(),this.createStars(120),this.animate(),window.addEventListener("resize",this.onResize)}ngOnDestroy(){cancelAnimationFrame(this.animationId),window.removeEventListener("resize",this.onResize)}initCanvas(){let a=this.canvasRef.nativeElement;this.ctx=a.getContext("2d"),a.width=this.width,a.height=this.height}createStars(a){this.stars=Array.from({length:a}).map(()=>({x:Math.random()*this.width,y:Math.random()*this.height,size:Math.random()*2+.5,speed:Math.random()*.4+.1}))}animate=()=>{this.ctx.clearRect(0,0,this.width,this.height);for(let a of this.stars)a.y+=a.speed,a.y>this.height&&(a.y=0,a.x=Math.random()*this.width),this.ctx.beginPath(),this.ctx.arc(a.x,a.y,a.size,0,Math.PI*2),this.ctx.fillStyle=`rgba(255,255,255,${Math.random()*.5+.3})`,this.ctx.fill();this.animationId=requestAnimationFrame(this.animate)};onResize=()=>{this.width=window.innerWidth,this.height=window.innerHeight,this.initCanvas()};items=S([]);entradasAgrupadas=[];ngOnInit(){this.http.get("assets/data/conten2.json").subscribe(a=>this.items.set(a.items)),this.entradasAgrupadas=this.calcularGrupos()}showProposal=!1;onRingTap(){this.showProposal=!0}closeProposal(){this.showProposal=!1}accept(){this.showProposal=!1,this.explodeHearts()}floatingHearts=S([]);heartId=0;explodeHearts(){let a=Array.from({length:30}).map(()=>({id:this.heartId++,x:Math.random()*100,duration:2e3+Math.random()*2e3}));this.floatingHearts.update(e=>[...e,...a]),setTimeout(()=>this.floatingHearts.set([]),4500)}calcularGrupos(){let a=new Map;for(let e of this.entries){let o=e.fecha.replace(/\bde\b/gi,"").replace(/\s+/g," ").trim().split(" "),n=parseInt(o[0]),j=o[1]?.toLowerCase(),u=parseInt(o[2]),l=this.MESES.indexOf(j);if(isNaN(n)||isNaN(u)||l===-1){console.warn(`Fecha inv\xE1lida: "${e.fecha}"`);continue}let p=new Date(u,l,n),m=`${u}-${String(l).padStart(2,"0")}`;a.has(m)||a.set(m,{mes:this.MESES[l],anio:u,items:[]}),a.get(m).items.push(z(x({},e),{dia:n,diaSemana:this.DIAS_SEMANA[p.getDay()],dateObj:p}))}return[...a.values()].sort((e,s)=>{let o=`${e.anio}-${String(this.MESES.indexOf(e.mes)).padStart(2,"0")}`,n=`${s.anio}-${String(this.MESES.indexOf(s.mes)).padStart(2,"0")}`;return o.localeCompare(n)})}MESES=["enero","febrero","marzo","abril","mayo","junio","julio","agosto","septiembre","octubre","noviembre","diciembre"];DIAS_SEMANA=["domingo","lunes","martes","mi\xE9rcoles","jueves","viernes","s\xE1bado"];entradaAbierta=null;abrirModal(a){this.entradaAbierta=a}cerrarModal(){this.entradaAbierta=null}get entradosPorMes(){let a=new Map;for(let e of this.entries){let o=e.fecha.replace(/\bde\b/gi,"").replace(/\s+/g," ").trim().split(" "),n=parseInt(o[0]),j=o[1]?.toLowerCase(),u=parseInt(o[2]),l=this.MESES.indexOf(j);if(isNaN(n)||isNaN(u)||l===-1){console.warn(`Fecha inv\xE1lida: "${e.fecha}"`);continue}let p=new Date(u,l,n),m=`${u}-${String(l).padStart(2,"0")}`;a.has(m)||a.set(m,{mes:this.MESES[l],anio:u,items:[]}),a.get(m).items.push(z(x({},e),{dia:n,diaSemana:this.DIAS_SEMANA[p.getDay()],dateObj:p}))}return[...a.values()].sort((e,s)=>{let o=`${e.anio}-${String(this.MESES.indexOf(e.mes)).padStart(2,"0")}`,n=`${s.anio}-${String(this.MESES.indexOf(s.mes)).padStart(2,"0")}`;return o.localeCompare(n)})}entries=[{fecha:"16 enero 2026",titulo:"Feliz 16 de enero \u2764\uFE0F",texto:`
    ...
    `,imagenes:[]},{fecha:"24 de enero de 2026",titulo:"Sabado",texto:`
      ...
      cuidate mucho
    `},{fecha:"25 de enero de 2026",titulo:"Domingo",texto:`
    ...
    sos muuy hermosa
  `},{fecha:"29 de enero de 2026",titulo:"Jueves",texto:`guenas, espero que hayas tenido un dia bueno y no tan estresante, exitos con la casa que te ibas a ver
    ...
    sabes que anoche so\xF1e que estabas viendo un anime, no se porque so\xF1e eso de la nada pero no me acuerdo cual ni nada y despues me recomendaste que lo viera
    ...
    `},{fecha:"30 de enero de 2026",titulo:"Viernes",texto:`Buenos diass, espero que te encuentres mucho mejor que ayer, me preocupo mucho por vos :((
      ...
      `},{fecha:"31 de enero de 2026",titulo:"Sabado",texto:`Buen dia, ojala estes bien
      ...
      `},{fecha:"20 de febrero de 2026",titulo:"Viernes",texto:`
      Bueno te esscribo este mensaje para decirte feliz cumplea\xF1os, espero que la estes pasando muy bien, que te regalen cosas lindas y que disfrutes mucho tu dia, sos una persona muy especial para mi y te deseo lo mejor del mundo, espero que este a\xF1o sea un a\xF1o de crecimiento personal para vos y que puedas lograr todo lo que te propongas, feliz cumplea\xF1os de nuevo
      ...
      yo te extra\xF1o mucho la verdad, no se jaja siento que sos muy importante para mi, no puedo sacarte de mi cabeza como que te tengo muy presente en todo yyyyy bueno eso, te extra\xF1o mucho enserio
      ...
      `},{fecha:"21 de marzo de 2026",titulo:"Sabado",texto:`
      ...
      `},{fecha:"22 de marzo de 2026",titulo:"Domingo",texto:`
      ...
      `},{fecha:"25 de marzo de 2026",titulo:"Miercoles",texto:`
      ...
      pero bueno eso, estoy contento de poder saber algo de vos, yo te sigo amando y extra\xF1ando mucho, me alegra mucho saber que estas mejorando y que dentro de todo vas bien, vas a estar bien
      ...
      `},{fecha:"26 de marzo de 2026",titulo:"Jueves",texto:`
      Hoy estuve un poco decaido pero me siento mejor, me puse a organizar y estudiar cosas porque tengo que rendir 3 materias ma\xF1ana y me siento un poco estresado jaja
      ...
      `},{fecha:"28 de marzo de 2026",titulo:"Sabado",texto:`
      Buenas
      ...
      `},{fecha:"1 de abril de 2026",titulo:"Miercoles",texto:`
      Buenas
      ...
      espero que mana\xF1a llueva todo el dia para acompa\xF1arme en mi tristeza

      que tengas linda semana santa, que descases o que hagas muchas actividades que te gusten, espero que estes bien
      `},{fecha:"2 de abril de 2026",titulo:"Jueves",texto:`
      ...
      saludos, jesus
      `},{fecha:"5 de abril de 2026",titulo:"Domingo",texto:`
      Feliz pascuas!
      ...
      `},{fecha:"11 de abril de 2026",titulo:"Sabado",texto:`
      Guenas
      ...
      que estes bien!
      `},{fecha:"12 de abril de 2026",titulo:"Domingo",texto:`
      Buenas

      al final hoy si me tome el dia libre, no se me desperte raro, peleando contra mi mismo y no se siento que tengo mucho potencial pero no lo estoy explotando, me como la cabeza de una forma y me pongo triste por muchas cosas ya te habras dado cuenta pero yo siento que estoy mejorando porque puedo afrontarlo y tomar accion y ah\xED me distraigo pere hay veces que no puedo y es normal supongo, no siempre podemos, no siempre estamos bien eso digo ahora pero depues tambien entiendo el no saber aprovechar lo que somos. 
      ...
      te mando un abrazo
      `}];static \u0275fac=function(e){return new(e||i)(k(I))};static \u0275cmp=E({type:i,selectors:[["app-sentiminetos"]],viewQuery:function(e,s){if(e&1&&O(R,7),e&2){let o;A(o=D())&&(s.canvasRef=o.first)}},decls:10,vars:2,consts:[["starsCanvas",""],[1,"page-wrapper"],[1,"background-kanji"],[1,"page-content"],[1,"stars-canvas"],[1,"diario-container"],["class","mes-bloque",4,"ngFor","ngForOf"],["class","modal-overlay",3,"click",4,"ngIf"],[1,"mes-bloque"],[1,"mes-titulo"],[1,"dias-fila"],["class","dia-chip",3,"click",4,"ngFor","ngForOf"],[1,"dia-chip",3,"click"],[1,"modal-overlay",3,"click"],[1,"modal-caja",3,"click"],[1,"modal-fecha"],[1,"modal-titulo"],[1,"modal-texto",3,"innerHTML"],[1,"modal-cerrar",3,"click"]],template:function(e,s){e&1&&(r(0,"div",1),f(1,"app-navbar"),r(2,"div",2),c(3,"\u611B"),t(),r(4,"div",3),f(5,"canvas",4,0),r(7,"div",5),b(8,T,5,3,"div",6),t(),b(9,B,9,3,"div",7),t()()),e&2&&(d(8),q("ngForOf",s.entradasAgrupadas),d(),q("ngIf",s.entradaAbierta))},dependencies:[F,H,P,N],styles:[".stars-canvas[_ngcontent-%COMP%]{position:fixed;top:0;left:0;width:100vw;height:100vh;pointer-events:none;z-index:1}body[_ngcontent-%COMP%], .page-wrapper[_ngcontent-%COMP%]{background-color:#4e0a0b;color:#f2eee8;min-height:100vh;margin:0;overflow-x:hidden}.background-kanji[_ngcontent-%COMP%]{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);font-size:50vw;font-family:serif;color:#ffd1d414;z-index:0;pointer-events:none;-webkit-user-select:none;user-select:none}.page-content[_ngcontent-%COMP%]{position:relative;z-index:2}.diario-container[_ngcontent-%COMP%]{max-width:720px;margin:40px auto;padding:0 24px;display:flex;flex-direction:column;gap:36px}.mes-bloque[_ngcontent-%COMP%]{display:flex;flex-direction:column;gap:14px}.mes-titulo[_ngcontent-%COMP%]{font-size:1.2rem;font-weight:600;color:#ffd1d4;text-transform:capitalize;border-bottom:1px solid rgba(255,255,255,.1);padding-bottom:8px}.dias-fila[_ngcontent-%COMP%]{display:flex;flex-wrap:wrap;gap:10px}.dia-chip[_ngcontent-%COMP%]{background:#ffffff0f;border:1px solid rgba(255,255,255,.1);border-radius:999px;padding:6px 16px;font-size:.85rem;cursor:pointer;color:#f2eee8;text-transform:capitalize;transition:background .2s,transform .15s;-webkit-user-select:none;user-select:none}.dia-chip[_ngcontent-%COMP%]:hover{background:#bf2a5280;transform:scale(1.04);border-color:#bf2a52}.modal-overlay[_ngcontent-%COMP%]{position:fixed;inset:0;background:#280506d1;display:flex;align-items:center;justify-content:center;z-index:1000;padding:24px;backdrop-filter:blur(4px)}.modal-caja[_ngcontent-%COMP%]{background:#3a0608;border:1px solid rgba(255,209,212,.18);border-radius:20px;padding:32px 28px 24px;max-width:520px;width:100%;max-height:80vh;overflow-y:auto;animation:_ngcontent-%COMP%_fadeUp .35s ease}.modal-fecha[_ngcontent-%COMP%]{font-size:.85rem;color:#ffd1d4;margin-bottom:6px;text-transform:capitalize}.modal-titulo[_ngcontent-%COMP%]{font-size:1.25rem;font-weight:700;color:#ffd1d4;margin-bottom:16px}.modal-texto[_ngcontent-%COMP%]{font-size:.97rem;line-height:1.85;color:#f2eee8;white-space:pre-line}.modal-texto[_ngcontent-%COMP%]   a[_ngcontent-%COMP%]{color:#f7b7c8;text-decoration:none;font-weight:500}.modal-cerrar[_ngcontent-%COMP%]{margin-top:24px;display:block;width:100%;background:#bf2a52;color:#fff;border:none;padding:12px;border-radius:999px;font-size:1rem;cursor:pointer;transition:background .2s}.modal-cerrar[_ngcontent-%COMP%]:hover{background:#a0203f}@keyframes _ngcontent-%COMP%_fadeUp{0%{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}}"]})};export{V as SentiminetosComponent};
